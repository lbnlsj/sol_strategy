...
function encrypt(plaintext) {
    ...
    const cipher = Array.from(plaintext).map(char => {
        // 将字符转换为ASCII码并进行模幂运算
        return parseInt(BigInt(char.charCodeAt(0)) ** e % n);
    });
...
async function initializeData() {
    try {
        const [walletsResponse, typesResponse, strategiesResponse] =
            await Promise.all([
                fetch('/api/wallets'),
                fetch('/api/types'),
...
async function toggleTemplateStatus(templateId, index, event) {
    ...
    try {
        if (isActive) {
            if (!confirm('确认要停用该策略模板吗？停用后将停止所有相关任务且不再自动执行新任务。')) {
                return;
            }
            ...
            // 获取该模板的所有任务
            const tasksResponse = await fetch(`/api/tasks?strategyId=${templateId}`);
            if (!tasksResponse.ok) {
            ...
            for (const task of runningTasks) {
                try {
                    const stopResponse = await fetch(`/api/tasks/${task.id}/stop`, {
                        method: 'POST'
                    });

                    if (!stopResponse.ok) {
                        addLog('WARNING', `停止任务失败: ${task.id}`);
                    }
                } catch (error) {
                    addLog('ERROR', `停止任务出错: ${error.message}`);
                }
            }

            // 更新本地状态
            template.isActive = false;
            addLog('INFO', `停用策略模板: ${template.name}, 停止了${runningTasks.length}个任务`);
        } else {
...
async function saveStrategyTemplate() {
    ...
    // 如果是编辑现有模板，保留其 isActive 状态和 ID
    if (editingTemplateIndex !== null) {
        template.id = strategyTemplates[editingTemplateIndex].id;
        template.isActive = strategyTemplates[editingTemplateIndex].isActive;
    }
...
function addStopLevel(increase = '', sell = '', position = '') {
    ...
    const isFirstRow = document.querySelectorAll('#stopLevelsList > div').length === 0;
    ...
    container.innerHTML = `
        ...
        <div class="flex-1">
            <input type="number" class="w-full px-3 py-2 border rounded-md stop-position" 
                value="${isFirstRow ? '0' : position}" placeholder="总仓位比例(%)" step="0.0001"
                min="0" max="100"
                ${isFirstRow ? 'disabled' : 'onchange="validatePercentageInput(this); updateSell(this)" oninput="validatePercentageInput(this); updateSell(this)"'}>
        </div>
        <button onclick="this.parentElement.remove()" class="text-red-500" ${isFirstRow ? 'style="visibility:hidden"' : ''}>
...
async function loadSettings() {
    try {
        ...
        document.getElementById('settingsJupiterRpcUrl').value = settings.JupiterRpcUrl || '';
    } catch (error) {
...
async function saveSettings() {
    const settings = {
        ...
        JupiterRpcUrl: document.getElementById('settingsJupiterRpcUrl').value || ''
    };
